...
export function DroppableColumn({
  ...
  /** Real number of reports in this status, which can exceed what is loaded so far. */
  total?: number
  hasMore?: boolean
  loadingMore?: boolean
  onLoadMore?: () => void
  ...
  }, [hasMore, onLoadMore, reports.length])
  ...
  return (
    <div ref={setNodeRef} className="flex flex-col rounded-2xl border bg-muted/30 p-4 min-h-[500px]">
      <div className="flex items-center justify-between mb-4 pb-2 border-b">
        ...
        <Badge variant="secondary">{total ?? reports.length}</Badge>
      </div>
            ...
            <>
              ...
              {hasMore && (
                ...
                >
                  ...
                  {loadingMore ? "Loading..." : `${(total ?? reports.length) - reports.length} more`}
                </div>
              )}
            </>
          )}
